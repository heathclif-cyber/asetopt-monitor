...
function buildCashSheet(
  ...
  months.forEach((m, mi) => {
    ...
    const subs = [
      'Target', 'Kompensasi', 'Denda', 'PPN', 'PPH', 'PBB', 'Jaminan', 'No Dok (SAP)', 'Total Diluar Jaminan', '%',
    ]
    subs.forEach((s, j) => {
  ...
  ws.getCell(rIdx, 1).value =
    'Catatan: Kompensasi/PPN/PPH dialokasi proporsional dari pembayaran invoice. Denda dari cash_in jenis denda. Jaminan belum ditrack di AsetOpt (kosong). Satuan Rp 000.'
